...
const nextConfig = {
  ...
  async redirects() {
    return [
      ...
      // ── ?scroll= query param → base page (301) ───────────────────────────
      // These ?scroll= URLs were generated by an older version of the site.
      // Google indexed them as separate pages causing "Alternative page with
      // proper canonical tag" errors in Search Console.
      // 301 redirecting them to the base page tells Google to permanently
      // drop the ?scroll= variants and consolidate to the canonical URL.

      // Destinations scroll variants
      {
      ...
      // Eliminates duplicate content from old ID-based package URLs.
